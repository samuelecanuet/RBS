...
map<string, map<double, map<string, double>>> ScalePart_MAP;
map<double, map<string, double>> XSAl_MAP;
map<double, map<string, double>> XSOxygen_MAP;
map<double, map<string, double>> XSCarbon_MAP;
...
double FunctionToMinimize(const double *par)
{
    ...
    Calibration_Offset_MAP[3.0] = par[0];
    // Calibration_Offset_MAP[3.0] = 0;
    Calibration_Coefficient_MAP[3.0] = 2*par[1];
        ...
        for (double energy : Energy)
        {
            for (string face : Face)
            {
                ...
                if (!Sim_Hist_conv_MAP[type][energy][face])
                {
                    ...
                    Al1->Scale(XSAl_MAP[energy][face] / Al1->Integral() * coef * ScalePart_MAP[type][energy][face] * (int)Thickness_Al1_MAP[type]);
                    Oxygen->Scale(XSOxygen_MAP[energy][face] / Oxygen->Integral() * coef * ScalePart_MAP[type][energy][face] * (int)Thickness_Mylar_MAP[type]);
                    Carbon->Scale(XSCarbon_MAP[energy][face] / Carbon->Integral() * coef * ScalePart_MAP[type][energy][face] * (int)Thickness_Mylar_MAP[type]);
                    Al2->Scale(XSAl_MAP[energy][face] / Al2->Integral() * coef * ScalePart_MAP[type][energy][face] * (int)Thickness_Al1_MAP[type]);
...
// - THICK	3	B	0.910434
    ...
    Thickness_Mylar_MAP["THICK"] = 5900;
    Thickness_Al2_MAP["THICK"] = 110;

    //moi
    Calibration_Offset_MAP[1.2] = 78.03;  //4.19 3.19 1.25 1.15 3.48 4.25
    Calibration_Coefficient_MAP[1.2] = 1.86;
    ...
    ////// part //////
    ScalePart_MAP["THIN"][1.2]["A"] = 1.2e-1;
    ScalePart_MAP["THIN"][3.0]["A"] = 2e-1;
    ScalePart_MAP["THICK"][1.2]["A"] = 1.2e-1;
    ScalePart_MAP["THICK"][3.0]["A"] = 2e-1;
    ScalePart_MAP["THIN"][1.2]["B"] = 1.2e-1;
    ScalePart_MAP["THIN"][3.0]["B"] = 2e-1;
    ScalePart_MAP["THICK"][1.2]["B"] = 1.2e-1;
    ScalePart_MAP["THICK"][3.0]["B"] = 2e-1;
    /////////////////

    ////// XS //////
    XSAl_MAP[1.2]["A"] = 190 * 2.7/1.39; //from 150 to 190
    XSOxygen_MAP[1.2]["A"] = 123;
    XSCarbon_MAP[1.2]["A"] = 170*2.5;

    XSAl_MAP[3.0]["A"] = 50* 2.7/1.39;
    XSOxygen_MAP[3.0]["A"] = 78;
    XSCarbon_MAP[3.0]["A"] = 77*2.5;

    XSAl_MAP[1.2]["B"] = 210* 2.7/1.39; // from 190 to 230
    XSOxygen_MAP[1.2]["B"] = 140;
    XSCarbon_MAP[1.2]["B"] = 170*2.5;

    XSAl_MAP[3.0]["B"] = 50* 2.7/1.39;
    XSOxygen_MAP[3.0]["B"] = 82;
    XSCarbon_MAP[3.0]["B"] = 82*2.5;
    /////////////
