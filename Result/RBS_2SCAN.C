...
int main(int argc, char *argv[])
{
    ...
    string del = "rm saved_scan/Correlation_Saved_" + to_string(first) + "_" + to_string(second) + ".root";
    system(del.c_str());
    string command = "mv saved_scan/Correlation_Saving_" + to_string(first) + "_" + to_string(second) + ".root saved_scan/Correlation_Saved_" + to_string(first) + "_" + to_string(second) + ".root";
    system(command.c_str());

    TFile *f_save = new TFile(("saved_scan/Correlation_Saved_" + to_string(first) + "_" + to_string(second) + ".root").c_str(), "READ");
    ...
    double chi2_saving;
    TFile *f_saving = new TFile(("saved_scan/Correlation_Saving_" + to_string(first) + "_" + to_string(second) + ".root").c_str(), "RECREATE");
    ...
    for (double par1 = par_start[first]; par1 < par_stop[first]; par1 += par_step[first])
    {
        for (double par2 = par_start[second]; par2 < par_stop[second]; par2 += par_step[second])
        {
            ...
            if (Reader->GetEntries() > counter_progress)
            {
                while (Reader->Next())
                {
    ...
    f_saving->cd();
    H_Correlation[first][second]->Write();
    f_saving->Close();
